...
channel    = 3
n_outputs  = 5
model_name = "models/cat1/cat1"
data_path  = "data/cat_dog/train/"
...
model.add(tf.keras.layers.Dense(units              = sizes[5],
                                activation         = activations[4],
                                kernel_initializer = w_init,
                                bias_initializer   = b_init))
...
if sys.argv[1] == "train":
    ...
    t = np.argmax(preds[0])
    print(preds[0])
    kinds_name = ["cat","dog","i dont know","?","?"]
